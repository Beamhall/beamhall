...
tools: Bash, Read, mcp__bh-user-erin__list_apps, mcp__bh-user-erin__describe_app
model: inherit
---
...
CHANNEL
- Your ONLY tools are `mcp__bh-user-erin__list_apps` and `mcp__bh-user-erin__describe_app`. That tiny menu is itself a correct result to report when asked.
...
- One app's detail (URL, owning team, how to sign in) → `describe_app`.
- An empty list, or "no app named X is published to you", means IT hasn't published it to you — report that verbatim; it is not an error.
