...
**MCP-first management surface (built; the web Admin console is now second-tier).** The `admin_*` family is the primary management surface and covers the full lifecycle, not just create. Routine (autonomous, audited): `admin_update_beamhall` (quota/live-slots/status suspend·archive·reactivate/metadata), `admin_revoke_membership`, `admin_set_membership_role`, `admin_set_identity_status` (per-principal kill switch — PEP-enforced), `admin_set_user_enabled` + `admin_remove_user_from_group` (bundled-IdP offboarding, behind the `identityadmin.Provider` seam), `admin_list_releases` (rollback targets), the **audit read surface** `admin_query_audit` + `admin_verify_audit_chain` (the regulated hash-chained log, previously web-console-only), and `admin_backup_now` + `admin_list_backups` (online snapshot via `WithBackup`). Sensitive (four-eyes, reusing the `admin_action_requests` flow with new `AdminActionType`s): `admin_set_security_context` (runtime-class — weakens isolation), `admin_unfederate_directory`, `admin_prune_audit` (destroys tamper-evidence), `admin_restore_backup` (verifies + returns the operator runbook; never a live in-process overwrite). All it_admin-gated, audited, thin over the orchestrator. Still deferred: IdP user/group hard-delete, self-upgrade.
