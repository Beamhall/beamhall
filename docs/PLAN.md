...
### 5.10 Provisioned auth (beam SSO) — a beam inherits Beamhall's IdP (built; lab-verified 2026-06-23, full lifecycle pass 2026-06-24)

A builder gives their beam **company sign-in** with one tool call, the same way `create_database` gives it a database: no IdP config, **no credential ever reaching the agent**. The beam becomes an OIDC relying party against the **same bundled Keycloak** Beamhall already uses, so end-users log in with the accounts they already have. Decided 2026-06-23 via a design workflow (4 stances → judge panel → synthesis); design partner shaped the group model.

**Scope boundary (must be unmistakable in the tool copy).** `provision_auth` is for **corporate/employee SSO via the IdP Beamhall owns** — "log in with your company account." It is **NOT** a builder-facing way to stand up a *public / customer self-signup* account system. That population (anonymous signups, candidate portals, B2C) stays **app-managed**: the app owns its own users table (via `create_database`) and its own login flow. The two coexist in one beam (e.g. an `/admin` UI behind `provision_auth` + a `/portal` behind the app's own auth). This boundary matters because agents, seeing "auth offered by Beamhall," tend to reach for it to build *any* login facility; the description must steer them off that.

**Locked decisions (v1):**
- **In-app library mode only.** The beam runs a stock OIDC code+PKCE flow against the discovery doc. Gateway forward-auth (zero app code) was designed but is **out of v1 scope** (net-new Caddy `forward_auth` modeling + fail-open hazards on the atomic config render; revisit later).
- **Per-channel clients.** A `…-preview` client on `provision_auth`; a `…-live` client mirrored by `reconcileLiveResources` on first promote — mirrors the DSN channel model. A leaked preview secret can never touch production (distinct secret + distinct `aud`).
- **Audience isolation is the load-bearing security invariant.** App clients carry `aud=<own client_id>` only; they are **never** assigned the `beamhall-audience` client scope, and `CreateClient` **post-asserts** no mapper/scope injects the Beamhall resource URI (`cfg.Audience`, default `https://<base-domain>/mcp`) and refuses otherwise. Because the backplane requires `aud ⊇ cfg.Audience` (`auth.go:128`), an app-minted token is 401'd at the MCP door — **no privilege escalation**. Confidential clients, PKCE S256, `serviceAccounts/directGrants/implicit=false`, `fullScopeAllowed=false`, short token TTL.
- **Synced exact redirect URIs (no wildcard).** `Provider.SyncRedirectURIs` re-asserts the exact allowlist on every URL event (set-and-replace, so a dead rotated preview host stops being a valid callback). The allowlist lives in Keycloak, not the injected secret, so this is an immediate **no-redeploy** Admin call. Creds (`OIDC_ISSUER`/`OIDC_CLIENT_ID`/`OIDC_CLIENT_SECRET`) are host-independent, sealed **at provision time** (so they ride the *next* deploy's secretRefs snapshot — `beam.go:219` freezes refs before the host is minted; sealing in the first-deploy hook would be an off-by-one). The app derives `redirect_uri` from the request `Host`, so one image works across every rotating preview URL and on live.
- **Shared `beamhall` realm.** End-users are the company directory (incl. federated LDAP/AD). No isolated realm in v1 (the public/customer population is app-managed, so the isolated-realm option found no v1 driver).
- **Admin-curated group exposure (separation of duties).** The builder's `provision_auth` exposes **zero** groups. An IT admin maps a per-beam allowlist with **`admin_set_auth_groups(beam, [...])`** (set-and-replace, audited). Implemented leak-proof via **client-roles-derived-from-groups**: a client role per allowed group + group→role mapping, surfaced as a flat `groups` claim — Keycloak only ever puts a client's own roles in that client's token, so no app can vacuum the org structure. (Raw group-membership mappers can't be allow-listed; that's why roles.)
- **No four-eyes** on `provision_auth` — it is `ScopeResourcesWrite`, a per-beam capability like `create_database`, and changes only one beam's sign-in (not appliance-wide auth topology). It is audited under a distinct `policy.ActionProvisionAuth`.

**Tool surface.** `provision_auth(beam)` (builder, `ScopeResourcesWrite`) → mints the preview client, seals the OIDC trio, returns only `{auth_mode, secret_keys, secret_files, issuer}`, idempotent; `show_auth(beam)` (builder, `ScopeResourcesRead`) → mode/clients/audience/redirect-URIs/mapped-groups, never secret values; `admin_set_auth_groups(beam, [...])` (IT, `admin:it`) → the curated group allowlist; `rotate_auth(beam, channel)` (fast-follow) → fresh per-channel secret for incident response.

**The fourth stable seam extends, not forks.** `identityadmin.Provider` gains six methods (`CreateClient`, `GetClientSecret`, `RotateClientSecret`, `SyncRedirectURIs`, `SetClientGroupRoles`, `DeleteClient`) with IdP-neutral `ClientSpec`/`Client` types; `Disabled{}` no-ops them with `ErrNotEnabled`. **BYO-IdP degrades closed**: `provision_auth` fails before any half-state, is hidden from the per-caller menu, and returns the exact `set_secret` fallback recipe (the app validates tokens from the corporate IdP itself). New `domain.ResourceAuthClient` resource rows (one per channel, `BackingHandle` = Keycloak client UUID); the six sealed secrets reuse channel-scoped `SecretRef` with **zero** change to `secretRefs`/inject. Lifecycle hooks (all guarded on `provider.Enabled()` + an `auth_client` resource): provision→seal (DeleteClient rollback on failure); finalizeActiveRelease/resume→redirect-sync; pause→empty allowlist; reconcileLiveResources→mirror live client; finalizeLiveRelease/rollback→live redirect-sync (idempotent); reclaimResources→DeleteClient + vault.Delete on archive/destroy (no orphan client/secret). Implementation: `internal/identityadmin`, `internal/orch/auth.go`, `internal/mcp/tools.go` + `admin.go`; a conformance persona asserting a preview-client token is **401'd** by `/mcp` is the threat-model sign-off artifact.

### 5.11 Facility brokers (the connector model) — design (2026-06-24)

The MVP gives a beam managed **data** (Postgres, §5.7) and **identity** (provisioned auth, §5.10) the *same* way: a per-beam, per-channel scoped capability sealed into the vault and file-injected, with Beamhall holding the privileged credential the agent never sees. The next capability expansion (the intro's "connectors", §6) generalizes that one pattern into a **facility** — a managed primitive a beam *inherits* (object storage, message delivery, an internal DB it doesn't own) whose backend may live **off the appliance box**: on another IT-controlled box, or behind a governed relay to an external provider.

**Motivation (operator, not theory).** Two pressures, one mechanism: (1) **scale without one huge VM** — IT onboards hundreds of builders/beams by moving stateful heavyweights (PG, object storage, mail) onto separate IT-controlled boxes (on-prem or remote); (2) **governed access to the otherwise-impossible** — capabilities the appliance can't host well itself (outbound email needs IP reputation), reached through a broker that holds the credential, scopes it, and audits it. Neither weakens the thesis: the trust boundary is **IT control + Beamhall mediation**, not physical co-location. A second IT box is *distributing* the hall; a brokered external provider is *governed* external access — the opposite of a beam reaching Mailgun directly with a key baked into its code.

**No SDK / no third-party plugin ABI (locked).** "Pluggable" means an **in-tree backend interface per facility** with a small set of implementations Beamhall ships, selected by **admin configuration** (the way `BEAMHALL_PG_ADMIN_DSN` selects today's PG backend) — *not* loaded third-party code. Untrusted connector code in the control plane (holding provider-admin credentials) is a supply-chain surface the threat model deliberately closes (§6: CNB-only, no agent Dockerfile); a connector ABI would reopen it. Consistent with §7C ("connector **marketplace**" is out — a curated in-tree set is not a marketplace). Revisit only on a written driver.

**The invariant — north side / south side.**
- **North (beam ↔ facility): stable.** The beam always reaches an **in-hall endpoint** (`db.internal`, `objectstore.internal`, `mail.internal`) over a protocol it already speaks, with a capability **scoped to that endpoint and meaningless outside the hall**. This never changes wherever the backend lives — it keeps the beam contract stable while the backend moves. It also refines "no credential reaches the beam" into the achievable rule: **the beam may hold only a capability valid *inside* the hall, never a credential valid *outside* it.**
- **South (facility ↔ backend): pluggable.** Where the backend lives, the privileged credential to it, how per-beam scoped capabilities are minted, and the egress to it — all south-side, all admin-configured, all invisible to the beam.

**The one engineering axis — in-path vs out-of-path.**
- **In-path** — a Beamhall-run stub/relay on the appliance the beam talks to, forwarding to the backend. Preserves the north invariant perfectly and makes the broker a **policy + audit chokepoint** (per-operation audit — a *new* capability vs. today's provision-granularity). Cost: the appliance sits in the data path (CPU/bandwidth).
- **Out-of-path** — the beam connects straight to the (IT-controlled) backend with a scoped credential over a Beamhall-managed egress route. Scales bandwidth off the box; audit drops to provision/connect granularity (no regression — local PG queries aren't audited today either) and the beam holds a routable credential.

**Protocol shape decides how much the broker can hide** (terminate-and-re-originate vs. pass-through):

| Facility | Protocol | Broker mode | Beam holds | Hides provider? | Residency options |
|---|---|---|---|---|---|
| Email | SMTP/HTTP, fire-and-forget | terminate & re-originate (always in-path) | nothing / a bridge cred | ✅ fully | relay → external provider **or** internal smarthost |
| Object store | HTTP, signed req/resp | terminate & re-sign (in-path) **or** scoped key (out) | a broker-scoped key | ✅ fully | first-party local / MinIO box / external S3 |
| Postgres / DB | stateful binary wire | pass-through / pool only | a scoped role cred | ⚠️ instance only | local / remote IT cluster / managed |

**Reuses existing seams; adds south-side duties.** Lifecycle is the §5.10/§5.8 contract unchanged — provision (preview) → mirror-on-promote (fresh live scope) → reclaim-on-destroy, the preview/live channel split, the `domain.Resource` row + channel-scoped `SecretRef` + file injection. New control-plane responsibility lives entirely south of the beam: Beamhall now custodies **south-side** credentials/egress (TLS/mTLS to backends, provider-cred rotation, backend health), and the egress allowlist becomes **facility-aware** (selecting a backend opens exactly its route, narrowly — never a manual broad hole). Honest scope: facilities offload stateful heavyweights, but **multi-host beams** are a *separate* axis (the `RuntimeDriver`, §5.3), not delivered by this seam.

**Realization — a shared service container per broker (decided 2026-06-24).** A facility broker runs as a **single shared service container** attached to each beamhall bridge — the `bh-postgres` precedent — reached by Docker DNS (`bh-mail`, future `bh-s3`). Beams therefore never touch the host and there is **no egress hole on the beam bridges** (container-to-container). The broker's engine is **Beamhall's own code** (the `beamhalld` binary in a relay mode), driven by the control plane over a one-directional internal channel — **beamhalld→broker only**, so there is no container→host hop:
- **config-push** — beamhalld pushes the provider config + per-beam registrations/policy to the broker's **loopback-published control port** (the `bh-postgres` loopback-admin precedent), bearer-token authed;
- **audit-pull** — beamhalld polls the broker's in-memory event ring and appends per-message events to the hash chain. (Residual: events buffered between pulls are lost if the broker crashes first — bounded by the poll interval; the security-decision audit is unaffected, it stays in beamhalld.)
- The broker's **outbound** to the external provider rides a *separate* non-restricted network (the default bridge), so allowing the provider never widens beam egress — only the broker reaches it.
- The control loop is **idempotent + self-healing**: beamhalld re-pushes provider+registrations on a ticker, so a restarted broker (in-memory registry) is rebuilt from the authoritative resource rows. Per-beam credentials are pushed as a **hash** (the plaintext is sealed in the vault for injection, never moved over the channel).

This shape is the reusable facility pattern: the S3 broker (`bh-s3`) is built the same way.

**First facility built: email delivery** (§5.12) — the cleanest case: the relay fully terminates SMTP, so the app needs zero credential and zero knowledge of the backend.

### 5.12 Email delivery facility (first §5.11 facility) — lab-verified 2026-06-25

Outbound email a beam inherits like a database: `provision_email(beam)` → the app sends via stock SMTP to an in-hall relay that forwards to a configured provider; the provider credential never reaches the beam, and the app never learns which provider sends.

**Locked decisions (v1, operator 2026-06-24).**
- **North = SMTP to the broker.** The beam gets `SMTP_HOST/PORT/USER/PASS` injected (host = the broker's bridge DNS name, default `bh-mail`) and uses stock SMTP — no SDK, no provider-specific code, no knowledge of the backend.
- **South = one SMTP-smarthost adapter.** The broker forwards via `net/smtp` to a configured smarthost — an external provider (Mailgun/SendGrid/SES/Postmark) **or** an internal corporate relay; one impl covers all, provider swap is config. Per-provider REST adapters deferred (only for template/batch features).
- **Broker = a shared `bh-mail` service container** (the `beamhalld` binary in `mail-relay` mode) attached to each beamhall bridge like `bh-postgres` — **§5.11 shared-service-container realization**. It runs the go-smtp engine (`internal/facility/mail`, `emersion/go-smtp` v0.24); beams reach it container-to-container (no host exposure, no beam-bridge egress hole). beamhalld drives it over the §5.11 control channel (config-push provider+registrations to the broker's loopback control port; audit-pull per-message events into the hash chain). The broker reaches the smarthost over the default bridge, so beams can't. A fuller MTA (maddy) is the upgrade path if Beamhall ever needs its own queue/retry/DKIM.
- **Provider config = env (`BEAMHALL_MAIL_*`), not an MCP tool.** The smarthost (host/port/user/pass/STARTTLS) is appliance-wide infra IT sets at install — exactly like `BEAMHALL_PG_ADMIN_DSN` configures the DB backend. This sidesteps persisting a global credential (which would either leak into beams via the shared-secret table or need a migration). `admin_set_email_provider` is therefore **dropped** for v1; add it later with a dedicated sealed store if runtime reconfig over MCP is wanted.
- **Outbound only.** Inbound/receiving mail (MX, routing, parsing) is out of v1.
- **Preview delivers like live (operator call).** Preview-channel mail goes to real recipients, same as live — no sandbox/redirect in v1. ⇒ submission creds are **per-beam, `ChannelShared`** (injected into both channels like user/beamhall secrets), so there is **no promote-time mirror** for email — simpler than the DB/auth facilities. Blast radius is bounded by the per-beam sender allowlist + rate-limit + per-message audit (below); a per-channel sandbox can be added later by splitting creds to channel-scoped, without breaking this design.

**Tool surface.** `provision_email(beam)` (builder, `ScopeResourcesWrite`) → mint the per-beam submission cred at the broker, seal `SMTP_HOST/PORT/USER/PASS` + the broker's STARTTLS cert `SMTP_CA` (ChannelShared), return keys/files only, idempotent — the beam starts with **no allowed senders** (IT curates); `show_email(beam)` (builder, `ScopeBeamhallsRead`) → host/port/username/allowed-senders/rate-limit, never the password; `admin_set_email_senders(beam, [...])` (IT, `admin:it`) → per-beam From allowlist (set-and-replace, mirrors `admin_set_auth_groups`). The server `Instructions` + entry-point copy steer agents off wiring Mailgun/SES directly (anti-shadow-IT).

**Governance (the in-path payoff).** The broker enforces the per-beam **From allowlist** (reject — anti-spoof across beams), a per-beam **rate-limit/quota** (token bucket; mail-bomb defense; protects the shared smarthost's IP reputation), and emits a **per-message audit event** (envelope from/rcpt-count/subject/size/message-id/result) that beamhalld pulls into the hash chain — per-operation visibility we lack for DB. Only the broker holds the provider credential and the egress route to the smarthost (its default-bridge outbound); beams reach only `bh-mail`. The per-beam SMTP creds the beam holds are **bridge-only** (valid only against the broker, meaningless outside the hall); `SMTP_PASS` is auto-scrubbed from logs. **Degrades closed (no smarthost configured):** `provision_email` fails before any half-state, is hidden from the per-caller menu, and returns the `set_secret` fallback recipe (the app brings its own SMTP creds).

**Lifecycle & seams (built).** New `domain.ResourceEmail` (ChannelShared; `Spec` holds username/pass_hash/senders/limits to rebuild the broker registry on boot; `BackingHandle` = the SMTP username); ChannelShared `SecretRef`s reuse `secretRefs`/inject **unchanged**. Hooks: provision→mint+register-at-broker+attach-bridge+seal (deregister rollback on failure); **promote→no-op** (shared creds already inject into live); `reclaimResources`→deregister at broker + `vault.Delete` on archive/destroy. Boot/periodic `ReconcileEmail` re-pushes provider+registrations (self-healing across broker restarts); `DrainEmailAudit` pulls events into the chain. Implementation: `internal/facility/mail` (engine + go-smtp relay + control API/server + control client), `internal/orch/email.go`, `internal/mcp` (`tools.go`/`admin.go`/`visibility.go`/`server.go`), `cmd/beamhalld` (`mail-relay` subcommand + wiring), `internal/config` (`BEAMHALL_MAIL_*`). **Status: code-complete + unit/race-tested + lab-verified 2026-06-25** (appliance full pass in `docs/lab-phase0-validation.md`: control channel, provision→bridge-attach, IT sender allowlist, deploy→`SMTP_*` injection, delivery to a sink, disallowed-sender 550, beam→smarthost-direct egress-denied while beam→`bh-mail` reachable, audit chain intact, destroy→broker deregister). **STARTTLS shipped + re-verified:** the broker offers STARTTLS with a **persisted self-signed cert** (stable across restarts), injected to beams as a 5th secret **`SMTP_CA`**; a strict Go `net/smtp` client does STARTTLS(verify `SMTP_CA`)→stock `PlainAuth`→send (proven live), and `AllowInsecureAuth` stays on so nodemailer/Python smtplib also work. Re-runnable conformance: `scripts/agent-conformance/email-delivery.sh` (8 steps, passes live); the broker topology is stood up by `scripts/mail-broker-setup.sh`. **Remaining:** fold the broker into the production installer/packaging.
...
- **Beam auth inheritance (a beam reuses Beamhall's IdP):** ~~can a builder give their app company sign-in without IdP config or credentials?~~ **BUILT + LAB-VERIFIED (2026-06-23) → `provision_auth` (§5.10).** Audience isolation proven end-to-end on the appliance (an app-client token is 401'd by `/mcp`; `auth-isolation.sh`). Per-beam OIDC client in the bundled Keycloak, creds sealed/file-injected like `create_database`, audience-isolated so an app token can't hit the backplane, redirect URIs auto-synced across preview rotation, admin-curated group exposure (`admin_set_auth_groups`). The **full lifecycle was driven live 2026-06-24** (`auth-redirect-sync.sh`): deploy→pause→resume sync the preview redirects, promote mirrors a distinct live client (own audience + own secret + carried group allowlist on the stable live host), destroy reclaims both channel clients. **v1 = in-app library mode, bundled-IdP only** (BYO-IdP degrades to a `set_secret` recipe); gateway forward-auth, isolated end-user realms, and `rotate_auth` designed but deferred. Scope boundary: corporate/employee SSO only — public/customer self-signup stays app-managed.
