...
- **Sensitive-admin approval flow:** the sensitive tier (`admin_federate_directory`, future restores/upgrades) currently **fails closed** unless `BEAMHALL_IDP_SENSITIVE_ADMIN=on`. Open: build the **four-eyes pending-approval** flow for it (mirror promotion approval) so a second IT operator confirms in-band rather than via a config flag.
