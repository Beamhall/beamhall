...
| 7 | IdP strategy | **Bundle Keycloak 26.4+, but validate *any* OIDC** (IdP-agnostic token validation; customer Okta/Entra/Keycloak pluggable by config) |
...
migrations/
```
**The two stable seams:** the high-level **MCP tool contract** and the **`RuntimeDriver` interface**. Both let Kubernetes and new clients arrive later without breaking the product promise. The MCP layer is a thin OAuth-gated translation surface; **all** policy/orchestration lives in the backplane (so the Admin UI and any future client reuse the same enforcement).
