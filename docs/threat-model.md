...
both dropped; on the bridge, only broker traffic passes — sibling workloads
cannot dial each other), the four-eyes promote gate (the
...
| **Secret theft** | No `get_secret` tool; `set_secret` write-only; age-encrypted at rest; file-injected at `/run/secrets` (never env); logs/metrics scrubbed; platform-sealed connection secrets (`SMTP_*`, `S3_*` endpoint config) cannot be overwritten or shadowed by user writes — `set_secret` refuses the sealed keys on a provisioned beam, and injection always prefers the most specific row | A compromised *running* workload can read its own injected secrets (by design — it needs them) | HSM/lease-based short-lived secrets | `TestAgentCannot/ReadSecretsBack`, `ObtainDatabaseCredentials`; scrubber proven on a real leak; `TestSetSecretRefusesFacilitySealedKeys`, `TestSecretInjectionPrefersBeamScopedRow` |
| **Lateral movement** | One Docker bridge per beamhall; **sibling workloads cannot dial each other even in-hall** (the intra-bridge exemption covers only the platform brokers, §7); the gateway refuses workload-originated requests to beam routes (403 — no cross-hall calls via public URLs, §7); app-to-app calls exist ONLY as IT-granted, per-call-audited relay calls (§7); db-per-beam scoped roles; secrets beam-scoped | A compromised app can call exactly the peers IT granted it, as itself | mTLS service mesh | `TestAgentCannot/EscapeItsBeamhall`, `ReachTheHostOrSiblingBeams`; `TestBeamToBeamEndToEnd` (sibling dial blocked, broker path intact, grant + revocation live); Postgres `42501` cross-db isolation (lab) |
...
| **Unauthorized beam-to-beam / relay abuse** (beam-to-beam, §5.15) | Apps have no network path to each other (see lateral movement); the ONLY app-to-app channel is the backplane relay: a dedicated listener (the one bridge→host port the INPUT guard opens), authenticating the calling WORKLOAD by an injected per-beam key (hash-looked-up, vault-sealed, `BEAMHALL_`-prefixed keys refused to `set_secret`) **plus** its live container address (a stolen key works from nowhere else — the address is read from the socket, never a header), re-checking the IT grant on every call, minting the standard assertion with `caller_type: beam`, capping sizes, rate and in-flight per source beam (A→B→A amplification bounded), and appending every call to the audit chain | A compromised app can call exactly its granted peers, as itself, within its rate — visible per-call in the audit log | Per-grant scoping to named tools | `internal/orch` relay suite (uniform refusals, revocation mid-flight, source-address mismatch, squat refusal); `internal/c2c` (XFF ignored); e2e `TestBeamToBeamEndToEnd` |
...
- **`BEAMHALL-INPUT` (iptables):** everything bridge-originated to the host is
  ...
  `/admin`, git) is unreachable from every workload. The relay port is a
  deliberate, authenticated hole: a dedicated listener that serves nothing
  but the relay and authenticates every request (see the relay row in §6).
